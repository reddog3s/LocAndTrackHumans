...
def setVisibility(gt_keypoints, headList):
    ...
    return human_gt_set_visibility
...
def compute_head_size_from_two_joints(humans, ind1, headList):
    """
...
def eval_ap_mpii_v2(humans_pred_set, conf_pred_set, humans_gt_set, gt_visibility_set, headList, neck_id, joint_names, thresh=0.5):
    """
    ...
    for i in range(len(humans_gt_set)):
        hsz_vec = compute_head_size_from_two_joints(humans_gt_set[i], neck_id, headList[i])
        ref_dist_set.append(hsz_vec)

    # consider all gt joints visible when no input is specified
    if len(gt_visibility_set) == 0:
        gt_visibility_set = setVisibility(humans_gt_set, headList)
